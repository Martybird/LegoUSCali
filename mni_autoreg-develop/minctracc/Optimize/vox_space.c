...
void build_reorder_matrix_vox2xyz(VIO_General_transform *trans, VIO_Volume volume) {
   ...
   axis = volume->spatial_axes[VIO_X]; //this is in world coordinates?
   if( axis >= 0 ) {
...
void build_reorder_matrix_xyz2vox(VIO_General_transform *trans, VIO_Volume volume) {
   ...
   axis = volume->spatial_axes[VIO_Z]; // what really is the axis here?
   if( axis >= 0 ) {
      Transform_elem( *lin, axis, 0 ) = 0.0;
      Transform_elem( *lin, axis, 1 ) = 0.0;
...
void get_into_slice_voxel_space(Arg_Data *globals,
                                 Voxel_space_struct *vox,
                                 int slice_index, VIO_Volume v2) {
   VIO_Transform 
      *lin;
   VIO_General_transform 
      *reorder, 
      *invertcali, // added by Xiao
      *w2v;
   VIO_Real 
     sign,
     tx,ty,tz;
   ...
   /* take care of the starting coordinate */
   convert_3D_world_to_voxel(globals->slices.slice_array[slice_index].slice_image,
                             globals->slices.slice_array[slice_index].start[VIO_X], // this is the world coordinates
                             globals->slices.slice_array[slice_index].start[VIO_Y],
                             globals->slices.slice_array[slice_index].start[VIO_Z],
                             &vox->start[0],    &vox->start[1],    &vox->start[2]);

   /* take care of the directions required to step throug h the volume */


   for(i=0; i<3; i++) {

    
     if (globals->slices.slice_array[slice_index].step[i] > 0.0) {
       sign = 1.0;
     }
     else{
       sign = -1.0;
     }
     convert_world_vector_to_voxel(globals->slices.slice_array[slice_index].slice_image,
        RVector_x(globals->slices.slice_array[slice_index].directions[i]) * sign, // this is in world coordinate
        RVector_y(globals->slices.slice_array[slice_index].directions[i]) * sign,
        RVector_z(globals->slices.slice_array[slice_index].directions[i]) * sign,
        voxel_vector);
     
     fill_Vector(vox->directions[i], voxel_vector[0],voxel_vector[1],voxel_vector[2]);
   }

   /* take care of the voxel-world * world-world * world-voxel transformation */
   ALLOC(w2v,1);
   ALLOC(invertcali,1);
   ALLOC(reorder,1);
   ...
   create_inverse_general_transform(get_voxel_to_world_transform( v2 ),
                                    w2v);
   create_inverse_general_transform(globals->trans_info.transformation,
                                    invertcali);
   
                                /* go from voxel to xyz space */
   build_reorder_matrix_vox2xyz(reorder, globals->slices.slice_array[slice_index].slice_image);
   
   // added invert - Xiao -> this is world-to-voxel for the US slice
   
                                   

   //swap the order of concatenation may do? -Xiao
   /*
   concat_general_transforms(reorder,
      get_voxel_to_world_transform( globals->slices.slice_array[slice_index].slice_image), 
      vox->voxel_to_voxel_space );
   concat_general_transforms(vox->voxel_to_voxel_space,
                             globals->trans_info.transformation,
                             vox->voxel_to_voxel_space );
   ...
   */
   concat_general_transforms(reorder, globals->trans_info.transformation, //invertcali,
      vox->voxel_to_voxel_space );
     // globals->trans_info.transformation,
    


   concat_general_transforms(vox->voxel_to_voxel_space, //w2vslice,
                             get_voxel_to_world_transform( globals->slices.slice_array[slice_index].slice_image),                         
                             vox->voxel_to_voxel_space );
